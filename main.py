...
<body>
    ...
    <div class="container">
        ...
        <div class="pozo-monto">{pozo_acumulado_fmt}</div>
        <div class="pozo-desc">
...
async def index():
    ...
    pozo_fmt = f"${app_state['pozo_acumulado']:,}".replace(',', '.')
    limite_fmt = f"${app_state['limite_cobertura']:,}".replace(',', '.')
    
    html = HTML_TEMPLATE.replace("{pozo_acumulado_fmt}", pozo_fmt)
    html = html.replace("{valor_bolillero}", str(app_state["valor_bolillero"]))
    html = html.replace("{valor_ruleta}", str(app_state["valor_ruleta"]))
    html = html.replace("{limite_cobertura_fmt}", limite_fmt)
