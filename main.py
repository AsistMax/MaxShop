...
app = FastAPI(title="MaxShop - Red de Comercios & Ahorro", version="9.2")
    ...
    <body class="bg-slate-950 text-slate-100 min-h-screen flex flex-col justify-between font-sans selection:bg-cyan-500 selection:text-slate-950">
        ...
        <main class="w-full max-w-md mx-auto px-4 py-6 space-y-6 flex-1">
            ...
            <div class="bg-gradient-to-br from-slate-900 via-slate-900 to-cyan-950/40 border border-slate-800 rounded-3xl p-5 shadow-xl space-y-4">
                <div class="flex justify-between items-center">
                    <span class="text-[10px] uppercase tracking-wider text-emerald-400 font-bold bg-emerald-950/80 px-2.5 py-0.5 rounded-full" id="lblEstadoPlan">Plan Gratuito Activo</span>
                ...
                <div class="flex justify-between items-center">
                    ...
                    <button type="button" id="btnVerPlanes" class="text-xs bg-cyan-500 hover:bg-cyan-400 text-slate-950 px-3.5 py-2 rounded-xl font-extrabold shadow-lg cursor-pointer">📋 Ver Planes</button>
                </div>

                <div class="grid grid-cols-2 gap-2 pt-2 border-t border-slate-800/80">
                    <button type="button" id="btnRecargaExpres" class="bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-300 p-2.5 rounded-2xl border border-cyan-500/30 text-center cursor-pointer">
                        <span class="block text-xs font-bold">⚡ Recarga Exprés</span>
                        <span class="block text-[10px] text-slate-400">+$10k por $500 (MP)</span>
                    </button>
                    <button type="button" id="btnPlanPro" class="bg-gradient-to-r from-cyan-400 to-blue-500 text-slate-950 p-2.5 rounded-2xl text-center font-bold cursor-pointer">
                        <span class="block text-xs font-black">⭐ Plan Pro</span>
            ...
            <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
                <span class="text-[10px] uppercase tracking-wider text-cyan-400 font-bold bg-cyan-950/80 px-2.5 py-1 rounded-full">Billetera Inteligente</span>
                <h2 class="text-xl font-bold text-white mt-2">Canjear Descuento en Comercio</h2>
                <div class="mt-4">
                    <button type="button" id="btnEscanearQR" class="w-full py-3.5 text-sm font-bold text-slate-950 bg-gradient-to-r from-cyan-400 to-blue-500 rounded-2xl cursor-pointer shadow-lg">📷 Escanear QR del Comercio</button>
                </div>
            </div>

            <div class="grid grid-cols-2 gap-3">
                <button type="button" id="btnSumarComercio" class="bg-slate-900 border border-slate-800 p-4 rounded-2xl text-left cursor-pointer">
                    <div class="text-cyan-400 text-xl mb-1">🏪</div>
                    <h3 class="text-xs font-bold text-white">Sumar mi Comercio</h3>
                </button>
                <button type="button" id="btnConocerBeneficios" class="bg-slate-900 border border-slate-800 p-4 rounded-2xl text-left cursor-pointer">
                    <div class="text-blue-400 text-xl mb-1">📋</div>
                    <h3 class="text-xs font-bold text-white">Beneficios</h3>
                </button>
            </div>
            ...
            <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 space-y-4 shadow-xl">
                ...
                <input type="text" id="inputBuscadorComercios" placeholder="Buscar por nombre, rubro o localidad..." class="w-full bg-slate-950 border border-slate-800 rounded-2xl px-4 py-2.5 text-xs text-white outline-none">
        ...
        <div id="authModal" class="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-50 hidden flex items-center justify-center p-4">
            <div class="bg-slate-900 border border-slate-800 w-full max-w-md rounded-3xl p-6 space-y-4 shadow-2xl">
                ...
                <div id="panelSesionContainer" class="space-y-4 hidden">
                    <p class="text-xs text-emerald-400" id="lblSesionInfo">Sesión activa</p>
        ...
        <div id="modalQR" class="fixed inset-0 bg-slate-950/95 z-50 hidden flex flex-col items-center justify-center p-4">
            <div class="bg-slate-900 border border-slate-800 w-full max-w-sm rounded-3xl p-5 space-y-4 text-center">
                <button type="button" id="btnCloseQR" class="text-slate-400 text-xs font-bold">Volver / Cerrar</button>
        ...
        <div id="modalMontoVenta" class="fixed inset-0 bg-slate-950/90 z-50 hidden flex items-center justify-center p-4">
            <div class="bg-slate-900 border border-slate-800 w-full max-w-sm rounded-3xl p-5 space-y-4 text-xs">
                ...
                <p id="lblInfoDescuentoComercio" class="text-cyan-300"></p>
                <input type="number" id="inputMontoCompra" placeholder="Monto total de compra ($)" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-white">
                ...
                <button type="button" id="btnCloseVenta" class="w-full py-2 bg-slate-800 text-white rounded-xl">Cancelar</button>
            </div>
        </div>
        ...
        <div id="modalComercio" class="fixed inset-0 bg-slate-950/80 z-50 hidden flex items-center justify-center p-4">
            <div class="bg-slate-900 border border-slate-800 w-full max-w-md rounded-3xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
                <div class="flex justify-between items-center border-b border-slate-800 pb-3">
                    ...
                    <button type="button" id="btnCloseComercio" class="text-slate-400 font-bold">✕</button>
                </div>
                <form id="formComercio" class="space-y-3 text-xs">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            <label class="text-[10px] text-cyan-400">Campaña Descuento (%)</label>
                            <input type="number" id="c_porcentaje_campana" value="20" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-white">
                        </div>
                        <div>
                            <label class="text-[10px] text-cyan-400">Días de Campaña</label>
                            <select id="c_dias_campana" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-white">
        ...
        <div id="modalAdmin" class="fixed inset-0 bg-slate-950/95 z-50 hidden flex items-center justify-center p-4">
            <div class="bg-slate-900 border border-slate-800 w-full max-w-4xl rounded-3xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
                <div class="flex justify-between items-center border-b border-slate-800 pb-3">
                    ...
                    <button type="button" id="btnCloseAdmin" class="text-slate-400 font-bold">✕</button>
                </div>
                <div class="flex space-x-4 text-xs">
                    <button type="button" id="tabComercios" class="text-cyan-400 font-bold">Comercios</button>
                    <button type="button" id="tabUsuarios" class="text-slate-400 font-bold">Usuarios</button>
                </div>
                <div id="seccionComerciosAdmin" class="space-y-2 text-xs">
                    <a href="/api/admin/exportar/comercios" target="_blank" class="text-cyan-400 underline">Exportar Comercios CSV</a>
                    <div id="tablaComerciosAdminList">Cargando...</div>
                </div>
                <div id="seccionUsuariosAdmin" class="space-y-2 text-xs hidden">
                    <a href="/api/admin/exportar/usuarios" target="_blank" class="text-blue-400 underline">Exportar Usuarios CSV</a>
                    <table class="w-full text-left text-slate-300">
                        <thead><tr><th>Cliente</th><th>Correo</th><th>Plan</th><th>Saldo</th><th>Acción</th></tr></thead>
                        <tbody id="tablaUsuariosAdminBody"></tbody>
                    </table>
                </div>
            </div>
        </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                document.getElementById('btnVerPlanes').addEventListener('click', () => abrirModalAuth('registro'));
                document.getElementById('btnConocerBeneficios').addEventListener('click', () => abrirModalAuth('registro'));
                ...
                document.getElementById('btnSumarComercio').addEventListener('click', () => document.getElementById('modalComercio').classList.remove('hidden'));
                ...
                document.getElementById('btnCloseVenta').addEventListener('click', () => document.getElementById('modalMontoVenta').classList.add('hidden'));
                document.getElementById('btnCloseComercio').addEventListener('click', () => document.getElementById('modalComercio').classList.add('hidden'));
                ...
                document.getElementById('tabComercios').addEventListener('click', () => {
                ...
                document.getElementById('inputBuscadorComercios').addEventListener('input', (e) => {
            ...
            async function procesarAutenticacion(e) {
                ...
                if (modoRegistroAuth) {
                    ...
                    if(res.ok) {
                        mostrarToast("¡Cuenta creada con éxito y $50.000 de saldo inicial!");
            ...
            function renderizarComercios(comercios) {
                ...
                if(!comercios || comercios.length === 0) {
                    contenedor.innerHTML = `<div class="text-center text-slate-500">No hay comercios.</div>`;
                    return;
                }
                let html = '';
                comercios.forEach(c => {
                    html += `<div class="p-2.5 bg-slate-950 border border-slate-800 rounded-xl flex justify-between items-center">
            ...
            function iniciarEscaneoQR() {
                ...
                html5QrCode.start({ facingMode: "environment" }, { fps: 10, qrbox: 200 }, (decodedText) => {
                    ...
                    let cObj = listaComerciosGlobal.find(c => c.nombre_fantasias === decodedText);
                    let pct = cObj ? cObj.porcentaje_campana : 20.0;
                    let pctFinal = usuarioLogueadoGlobal.es_pro ? pct : (pct * 0.5);
                    window.porcentajeDescActual = pctFinal;
                    document.getElementById('lblInfoDescuentoComercio').innerText = `Descuento aplicable: ${pctFinal}%`;
            ...
            async function cargarDatosAdmin() {
                ...
                let json = await res.json();
                if(json.success) {
                    document.getElementById('tablaComerciosAdminList').innerHTML = json.comercios.map(c => `<div><b>${c.nombre_fantasias}</b> (${c.rubro})</div>`).join('') || 'Sin comercios';
                    document.getElementById('tablaUsuariosAdminBody').innerHTML = json.usuarios.map(u => `<tr><td>${u.nombre_completo}</td><td>${u.correo}</td><td>${u.es_pro ? 'PRO' : 'GRATIS'}</td><td>$${u.credito_descuento_disponible}</td><td><button onclick="adminCambiarPlan('${u.correo}', '${u.es_pro ? 'FREE' : 'PRO}')" class="text-cyan-400">Cambiar Plan</button></td></tr>`).join('');
                }
            }

            async function adminCambiarPlan(correo, nuevoPlan) {
                let res = await fetch('/api/admin/usuario/plan', { method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ correo: correo, nuevo_plan: nuevoPlan }) });
                if(res.ok) { mostrarToast("Plan actualizado"); cargarDatosAdmin(); }
            }
        </script>
    </body>
    </html>
