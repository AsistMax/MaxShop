...
# Planificador automático para el bolillero dominical
...
def sorteo_dominical_automatico():
    with Session(engine) as session:
        ...
        if participantes:
            ...
            print(f"[{datetime.now()}] ¡Sorteo dominical ejecutado con éxito! Ganador DNI: {ganador.dni_usuario}")
        else:
...
<body class="bg-[#0b0f19] text-white font-sans antialiased pb-12">
    ...
    <main class="max-w-md mx-auto p-4 space-y-8">
        ...
        <!-- Pozo Acumulado y Compra de Cartón -->
        <div class="bg-slate-900/80 border border-slate-800 p-6 rounded-2xl text-center shadow-xl">
            <span class="bg-orange-500/10 text-orange-400 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">🔥 Club de Beneficios</span>
            <h1 class="text-3xl font-black mt-4">Pozo Acumulado</h1>
            <p class="text-4xl font-extrabold text-amber-500 mt-2">$400,000</p>
            <p class="text-sm text-slate-400 mt-3">Disfruta de la red de comercios y participa por el bolillero dominical automático a las 19:00 hs.</p>
            
            <form action="/app/comprar-carton" method="POST" class="mt-6 space-y-3">
                <input type="text" name="dni" required placeholder="Ingresa tu DNI de Socio" class="w-full bg-slate-800 border border-slate-700 rounded-xl p-3 text-white text-sm focus:outline-none focus:border-amber-500">
                <button type="submit" class="w-full bg-gradient-to-r from-amber-500 to-orange-600 font-bold py-3 rounded-xl shadow-lg">Comprar Números ($1,000)</button>
            </form>
        </div>

        <!-- SECCIÓN RULETA -->
        <div id="ruleta-section" class="bg-slate-900/80 border border-slate-800 p-6 rounded-2xl shadow-xl space-y-6">
            <h2 class="text-xl font-bold text-center">Ruleta de Cobertura Max%Shop</h2>
            ...
            <form id="ruletaForm" action="/app/pagar-ruleta" method="POST" class="space-y-4">
                <input type="text" id="dniSocio" name="dni" required placeholder="Tu DNI de Socio" class="w-full bg-slate-800 border border-slate-700 rounded-xl p-3 text-white focus:outline-none focus:border-amber-500">
                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-500 font-bold py-3 rounded-xl shadow-lg transition-all">
                    GIRAR RULETA ($5,000)
                </button>
            </form>
        </div>
...
def pago_exitoso(tipo: str):
    ...
    <body class="bg-[#0b0f19] text-white flex items-center justify-center h-screen">
        <div class="bg-slate-900 border border-slate-800 p-8 rounded-2xl text-center max-w-md shadow-2xl">
            ...
            <p class="text-slate-300 mb-6">Tu pago para {tipo} se procesó correctamente.</p>
