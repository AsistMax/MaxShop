...
async def upload(file: UploadFile = File(...)):
    sb = get_supabase()
    if not sb: return JSONResponse({"success": False, "error":"No supabase configurado"}, 500)
...
async def registro(req: Request):
    try:
        sb = get_supabase()
        if not sb:
            return JSONResponse({"success": False, "error":"Supabase no configurado en Render"}, 500)
        d = await req.json()
        # Validacion basica
        if not d.get('correo') or not d.get('password'):
            return JSONResponse({"success": False, "error":"Falta correo y clave"}, 400)
        ...
        res = sb.table("usuarios").insert(nuevo).execute()
        return {"success": True, "usuario": nuevo}
    except Exception as e:
        print("ERROR REGISTRO:", str(e))
        return JSONResponse({"success": False, "error": f"Error DB: {str(e)}"}, 500)
...
async def crear_pago_recarga(req: Request):
    try:
        ...
        return JSONResponse({"success": False, "error": f"Error MP: {str(e)}"}, 500)
...
async def crear_pago_pro(req: Request):
    try:
        ...
        mp = get_mp()
        if not mp:
            return JSONResponse({"success": False, "error":"Falta MP_ACCESS_TOKEN en Render"}, 500)
        pref = {
