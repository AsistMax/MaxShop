...
async def girar_ruleta(request: Request):
    try:
        ...
        racha_final = 0 if racha >= 7 else racha
...
async def canjear_ruleta(request: Request):
    try:
        ...
        if mc < 100:
            return JSONResponse({"error": f"Necesitas 100 MaxCoins. Tenes {mc}"}, status_code=400)
...
async def girar_paga(request: Request):
    try:
        ...
        if supabase:
            res = supabase.table("usuarios").select("*").eq("correo", correo).execute()
            if res.data:
                credito_actual = float(res.data[0].get("credito_descuento_disponible") or 0) + premio
                supabase.table("usuarios").update({"credito_descuento_disponible": credito_actual}).eq("correo", correo).execute()
        return {"success": True, "premio": premio}
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)
...
async def crear_pago_recarga(request: Request):
    try:
        ...
        body = await request.json()
        pref = {
            "items": [{"title": "Ruleta MaxShop $500", "quantity": 1, "unit_price": 500}],
...
async def crear_pago_pro(request: Request):
    try:
        ...
        body = await request.json()
        pref = {
            "items": [{"title": "MaxShop PRO $5000", "quantity": 1, "unit_price": 5000}],
            "payer": {"email": body.get("correo")},
            "back_urls": {"success": f"{BASE_URL}/?pago=pro_ok", "failure": f"{BASE_URL}/?pago=error", "pending": f"{BASE_URL}/"},
            "auto_return": "approved",
            "metadata": {"correo": body.get("correo"), "tipo": "pro"}
        }
        result = sdk.preference().create(pref)
        return {"init_point": result["response"]["init_point"]}
    except Exception as e:
...
async def consumir_credito(request: Request):
    try:
        ...
        ahorro = monto * (0.20 if es_pro else 0.10)
        nuevo_credito = float(usuario.get("credito_descuento_disponible") or 0)
        if not es_pro:
            nuevo_credito = max(0, nuevo_credito - ahorro)
        supabase.table("usuarios").update({
...
async def upload_file(request: Request):
    return JSONResponse({"success": False, "error":"Usa URL directa"}, status_code=400)
