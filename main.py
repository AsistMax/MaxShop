...
# Cliente lazy para no crashear al iniciar
_supabase = None
def get_supabase():
    global _supabase
...
def health():
    sb = get_supabase()
    return {"status": "ok", "supabase_connected": sb is not None, "has_url": bool(os.getenv("SUPABASE_URL")), "has_key": bool(os.getenv("SUPABASE_KEY") or os.getenv("SUPABASE_SERVICE_ROLE_KEY"))}
...
def home(request: Request):
    try:
        ...
        return HTMLResponse(f"<h1>Error template: {e}</h1><p>Verifica que templates/index.html exista</p>", status_code=500)

@app.post("/api/upload")
async def upload(file: UploadFile = File(...)):
    sb = get_supabase()
    if not sb: return JSONResponse({"success": False, "error": "Supabase no conectado"}, status_code=500)
...
async def registro(req: Request):
    ...
    if ex.data: return JSONResponse({"success": False}, status_code=400)
...
async def consumir(req: Request):
    ...
    monto=float(d['monto_compra']); pct=float(com.data['porcentaje_descuento'] or 20)
    is_pro=usu.data.get('es_pro') or usu.data.get('suscripcion_activa')
    if not is_pro: pct=pct*0.5
    ahorro=monto*(pct/100); disp=float(usu.data.get('credito_descuento_disponible') or 0)
...
def admin():
    ...
    users=[]
    for u in (ru.data or []):
        u['es_pro']=u.get('es_pro') or False
        u['credito_descuento_disponible']=u.get('credito_descuento_disponible') or 0
        users.append(u)
    return {"success": True, "comercios": rc.data or [], "usuarios": users, "acciones": ra.data or []}
