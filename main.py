    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-16">
        ...
        <div id="ruleta" class="bg-gradient-to-r from-blue-950/40 via-[#101833] to-[#0A1128] border border-blue-500/40 rounded-3xl p-8 shadow-2xl space-y-8">
            ...
            <div class="bg-[#0A1128] border border-slate-800 p-8 rounded-2xl text-center space-y-6">
                <div class="ruleta-container">
                    ...
                    <div class="ruleta-centro">GIRAR</div>
                </div>
        ...
        <div id="comprar" class="bg-gradient-to-r from-emerald-950/30 via-[#101833] to-[#0A1128] border border-emerald-500/40 rounded-3xl p-8 shadow-2xl space-y-6">
            <div class="max-w-xl space-y-2">
                ...
                <p class="text-xs text-slate-400">Ingresa números separados por comas para cada línea. Al abonar ${valor_carton:,.0f}, se genera tu comprobante oficial.</p>
            </div>
...
async def girar_ruleta_app(dni: str = Form(...), nombre: str = Form(...)):
    ...
    return RedirectResponse(url=f"/?resultado_ruleta={urllib.parse.quote(premio_obtenido['label'])}#ruleta", status_code=303)

@app.post("/app/solicitar-comprobante-ruleta", response_class=HTMLResponse)
async def solicitar_comprobante_ruleta(dni: str = Form(...)):
    with Session(engine) as session:
        registros = session.exec(select(RegistroRuleta).where(RegistroRuleta.dni == dni)).all()
        if not registros:
...
async def admin_dashboard(username: str = Depends(verificar_admin), mensaje: str = None):
    ...
    ruleta_filas = "".join([f"""
        ...
        </tr>""" for r in registros_ruleta]) if registros_ruleta else '<tr><td colspan="5" class="p-4 text-center text-slate-500">Sin giros de ruleta registrados aún.</td></tr>'
        ...
        <div class="bg-[#101833] p-8 rounded-3xl border border-slate-800 space-y-6 shadow-2xl">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead><tr class="text-slate-400 border-b border-slate-800 text-xs"><th class="p-4">Nombre</th><th class="p-4">DNI</th><th class="p-4">Monto Póliza</th><th class="p-4">Comprobante Solicitado</th><th class="p-4">Fecha</th></tr></thead>
