...
# --- Inicializar FastAPI ---
app = FastAPI(title="MaxShop Pro - Red Nacional & Finanzas", version="10.0.0")
...
def read_root():
    ...
    <head>
        ...
        <title>MaxShop Pro | Red Federal, Descuentos & Finanzas</title>
    ...
    <body class="min-h-screen flex flex-col justify-between selection:bg-emerald-500 selection:text-slate-950">
        ...
        <!-- Barra de Información Financiera Superior (Estilo BullMarket / InvertirOnline) -->
        <div class="bg-slate-950 border-b border-slate-800/60 py-1.5 px-4 text-[11px] text-slate-400">
            <div class="max-w-6xl mx-auto flex flex-wrap justify-between items-center gap-2">
                <div class="flex items-center space-x-4">
                    <span class="flex items-center space-x-1 text-emerald-400 font-bold">🟢 <span>Dólar Blue Compra/Venta:</span> <strong class="text-white">$1.220 / $1.240</strong></span>
                    <span class="hidden md:inline text-slate-600">|</span>
                    <span class="hidden md:inline text-slate-300">Inflación Estimada Mensual: <strong class="text-emerald-400">2.8%</strong></span>
                </div>
                <div class="flex items-center space-x-3 text-cyan-400 font-semibold">
                    <span>🔥 Hot Sale & CashBack Activo en Toda la Red</span>
                </div>
            </div>
        </div>
                ...
                <div class="flex items-center space-x-3 cursor-pointer" onclick="switchSection('home')">
                    ...
                    <div>
                        <div class="flex items-center space-x-1.5">
                            ...
                            <span class="text-[8px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-1.5 py-0.5 rounded-full font-bold uppercase">PRO 10.0</span>
                        </div>
                    </div>
                </div>

                <div class="flex items-center space-x-3">
                    <!-- Botones de Historial: Avanzar / Regresar -->
                    <div class="hidden sm:flex items-center space-x-1 bg-slate-900 border border-slate-800 rounded-xl p-1">
                        <button onclick="history.back()" class="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition" title="Página Anterior">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
                        </button>
                        <button onclick="history.forward()" class="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition" title="Página Siguiente">
                    ...
                    <nav class="hidden md:flex items-center space-x-1 text-xs">
                        ...
                        <button onclick="switchSection('catalog')" id="navCatalog" class="px-3 py-2 rounded-xl text-slate-400 hover:text-white transition">Catálogo & Mural</button>
                        ...
                        <button onclick="switchSection('admin')" id="navAdmin" class="px-3 py-2 rounded-xl bg-amber-500/10 text-amber-400 border border-amber-500/30 font-bold transition">👑 Panel Admin</button>
                    </nav>
                </div>
            </div>
        </header>

        <!-- Contenido Deslizable Extensible -->
        <main class="max-w-6xl mx-auto px-4 py-8 w-full flex-grow space-y-12">
            ...
            <section id="secHome" class="space-y-10">
                <div class="glass-card rounded-3xl p-6 md:p-10 relative overflow-hidden shadow-2xl border-emerald-500/20 grid md:grid-cols-2 gap-6 items-center">
                    ...
                    <div class="space-y-4 relative z-10">
                        <span class="bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">Estrategia Federal MaxShop</span>
                        <h2 class="text-2xl md:text-4xl font-extrabold tracking-tight text-white leading-tight">Superá la inflación con <span class="brand-text-gradient">descuentos reales</span></h2>
                        <p class="text-slate-300 text-xs md:text-sm leading-relaxed">
                            Integramos la potencia de los comparadores de precios, beneficios bancarios y comercios adheridos de todo el país en una sola plataforma interactiva.
                        </p>
                        <div class="flex flex-wrap gap-3 pt-2">
                            <button onclick="switchSection('catalog')" class="brand-gradient hover:opacity-90 text-slate-950 font-extrabold px-5 py-3 rounded-xl text-xs transition shadow-lg">Ver Mural de Ofertas (%)</button>
                            <button onclick="switchSection('news')" class="bg-slate-900 hover:bg-slate-800 text-white font-semibold px-5 py-3 rounded-xl text-xs border border-slate-700 transition">Ver Análisis Financiero</button>
                        </div>
                    </div>
                    <div class="relative z-10 rounded-2xl overflow-hidden border border-slate-800 shadow-2xl">
                        <img src="https://images.unsplash.com/photo-1556742049-0a67d553c299?auto=format&fit=crop&w=800&q=80" alt="MaxShop Mural" class="w-full h-52 md:h-60 object-cover">
                    </div>
                </div>

                <!-- Carrusel Dinámico Ampliado -->
                <div class="space-y-3">
                    <div class="flex justify-between items-center px-1">
                        <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest">Tendencias, Bancos y Alianzas</h3>
            ...
            <!-- SECCIÓN CATÁLOGOS & MURAL DE DESCUENTOS (%) -->
            <section id="secCatalog" class="hidden space-y-6">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h2 class="text-2xl font-bold text-white">Mural Federal de Descuentos (%)</h2>
                        <p class="text-xs text-slate-400">Comercios adheridos y promociones agrupadas para comparar y ahorrar.</p>
                    </div>
                ...
                <div class="flex flex-wrap gap-2 text-xs">
                    ...
                    <button onclick="filterCategory('Indumentaria')" class="px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-300">Indumentaria</button>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6" id="realMerchantsContainer">
                    <div class="col-span-3 text-center text-slate-400 text-xs py-10 glass-card rounded-2xl">Cargando mural de ofertas...</div>
                </div>
            </section>

            <!-- SECCIÓN NOTICIAS, PROMOCIONES Y FINANZAS -->
            <section id="secNews" class="hidden space-y-6">
                <div class="space-y-2">
                    <h2 class="text-2xl font-bold text-white">Noticias, Inflación y Estrategias de Mercado</h2>
                    <p class="text-xs text-slate-400">Información clave para tomar decisiones financieras y aprovechar al máximo cada compra.</p>
                </div>

                <div class="grid md:grid-cols-3 gap-6">
                    <div class="glass-card p-5 rounded-2xl space-y-3">
                        <span class="text-[10px] bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded-md font-bold">ECONOMÍA</span>
                        <h3 class="font-bold text-sm text-white">Cómo rinden los descuentos frente a la inflación mensual</h3>
                        <p class="text-xs text-slate-400 leading-relaxed">Estrategias combinadas de reintegros bancarios y cuotas para proteger el poder de compra en supermercados.</p>
                    </div>
                    <div class="glass-card p-5 rounded-2xl space-y-3">
                        <span class="text-[10px] bg-cyan-500/10 text-cyan-400 px-2 py-0.5 rounded-md font-bold">TENDENCIAS HOT SALE</span>
                        <h3 class="font-bold text-sm text-white">Comparativa de Precios: MaxShop vs. Plataformas Masivas</h3>
                        <p class="text-xs text-slate-400 leading-relaxed">Análisis inteligente de costos de envío y comisiones para ofrecer mejores márgenes al comercio y mejores precios al socio.</p>
                    </div>
                    <div class="glass-card p-5 rounded-2xl space-y-3">
                        <span class="text-[10px] bg-amber-500/10 text-amber-400 px-2 py-0.5 rounded-md font-bold">FINANZAS & INVERSIÓN</span>
                        <h3 class="font-bold text-sm text-white">Opciones en Dólares y Cedears en el mercado actual</h3>
                        <p class="text-xs text-slate-400 leading-relaxed">Resumen rápido de instrumentos de renta fija y variable para potenciar los ahorros de tu membresía mensual.</p>
                    </div>
                </div>
            </section>
            ...
            <section id="secRegister" class="hidden space-y-6">
                <div class="max-w-2xl mx-auto space-y-6">
                    <div class="text-center space-y-2">
                        ...
                        <p class="text-xs text-slate-400">Súmate como socio o registra tu comercio con fotos desde tu dispositivo.</p>
                    </div>
                    ...
                    <div id="formUserBox" class="glass-card rounded-2xl p-6 shadow-xl space-y-4">
                        ...
                        <form id="userForm" class="space-y-3">
                            <div>
                                ...
                                <input type="email" id="userEmail" required placeholder="tu_correo@email.com" class="w-full bg-[#030712] border border-slate-800 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-emerald-500">
                            </div>
                        ...
                        <form id="merchantForm" class="space-y-3">
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <input type="text" id="mercName" required placeholder="Ej: Supermercado Norte" class="w-full bg-[#030712] border border-slate-800 rounded-xl px-3 py-2 text-xs">
                                </div>
                                <div>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <input type="number" step="0.1" id="discPercentage" required placeholder="Ej: 20" class="w-full bg-[#030712] border border-slate-800 rounded-xl px-3 py-2 text-xs">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-slate-400 mb-1">Título de Promo</label>
                                    <input type="text" id="discTitle" required placeholder="Ej: 20% OFF MaxShop" class="w-full bg-[#030712] border border-slate-800 rounded-xl px-3 py-2 text-xs">
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-400 mb-1">Subir Imagen desde el Celular o PC</label>
                                ...
                                <input type="url" id="mercImgUrl" placeholder="O pega una URL de imagen..." class="w-full mt-2 bg-[#030712] border border-slate-800 rounded-xl px-3 py-1.5 text-xs">
                            </div>
            ...
            <section id="secPay" class="hidden space-y-6">
                <div class="max-w-xl mx-auto glass-card rounded-2xl p-8 shadow-2xl border-emerald-500/30 relative">
                    ...
                    <div class="space-y-2 mb-6">
                        ...
                        <p class="text-xs text-slate-400">Verifica el estado del socio y aplica el descuento en el acto.</p>
                    </div>
                    <form id="paymentForm" class="space-y-4">
                        <div>
                            <label class="block text-xs font-medium text-slate-400 mb-1">Email del Socio</label>
                            <input type="email" id="payEmail" required placeholder="tu_correo@email.com" class="w-full bg-[#030712] border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm">
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" id="payAmount" required placeholder="Ej: 15000" class="w-full bg-[#030712] border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm">
                            </div>
                        </div>
            ...
            <section id="secAdmin" class="hidden space-y-6">
                <div class="max-w-4xl mx-auto glass-card rounded-2xl p-6 md:p-8 shadow-2xl border-amber-500/40 space-y-6">
                    <div class="flex justify-between items-center border-b border-slate-800 pb-4">
                        <div>
                            <h2 class="text-xl font-bold text-white">👑 Panel de Control Privado</h2>
                            <p class="text-xs text-slate-400">Control absoluto de registros, socios y pagos de membresías.</p>
                        </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        <div class="bg-[#030712] p-4 rounded-xl border border-slate-800 space-y-3">
                            <h3 class="font-bold text-xs text-emerald-400 uppercase tracking-wider">Socios Registrados</h3>
                            <div id="adminUsersList" class="space-y-2 max-h-60 overflow-y-auto text-xs pr-1"><span class="text-slate-500">Cargando...</span></div>
                        </div>
                        <div class="bg-[#030712] p-4 rounded-xl border border-slate-800 space-y-3">
                            <h3 class="font-bold text-xs text-cyan-400 uppercase tracking-wider">Comercios en la Red</h3>
        ...
        <footer class="border-t border-slate-800/80 py-6 text-center text-[10px] text-slate-500 space-y-2">
            ...
            <p class="text-slate-600">Entidades financieras compatibles: BNA+, Modo, Visa, Mastercard, Galicia, Santander, Macro, BBVA.</p>
        </footer>
        ...
        <script>
            ...
            function renderCatalog() {
                ...
                if(paginatedItems.length === 0) {
                    catalogContainer.innerHTML = `<div class="col-span-3 text-center text-slate-400 text-xs py-10 glass-card rounded-2xl">No hay comercios activos en esta página.</div>`;
            ...
            function switchSection(sectionId) {
                ['home', 'catalog', 'news', 'register', 'pay', 'admin'].forEach(s => {
            ...
            document.getElementById('merchantForm').addEventListener('submit', async (e) => {
                ...
                if(fileInput.files && fileInput.files[0]) {
                    ...
                    reader.onload = async function(uploadEvent) {
                        await sendMerchantToServer(name, cat, uploadEvent.target.result, perc, title);
                    };
            ...
            async function sendMerchantToServer(name, cat, imgUrl, perc, title) {
                ...
                try {
                    const res = await fetch(`/api/merchants/create?name=${encodeURIComponent(name)}&category=${encodeURIComponent(cat)}&image_url=${encodeURIComponent(imgUrl)}&lat=-34.6&lng=-58.3&title=${encodeURIComponent(title)}&percentage=${perc}`, { method: 'POST' });
                    const data = await res.json();
                    if(data.merchant_id) {
                        resDiv.innerHTML = `<div class="p-3 bg-cyan-950/40 border border-cyan-500/30 text-xs text-cyan-300">🏢 ¡Comercio y publicidad publicados en el mural con éxito!</div>`;
                        fetchMerchants();
                    }
                } catch(err) {
            ...
            async function loadAdminData() {
                ...
                try {
                    ...
                    mList.innerHTML = merchants.length === 0 ? '<span class="text-slate-500">Sin comercios.</span>' : '';
                    merchants.forEach(m => {
